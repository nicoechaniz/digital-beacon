"""Launchpad Mini control for digital-beacon.
...
logic) and adapted to:
  ...
  - OSC routing to both the SC beacon (32 BPFs, on/off per band) and the
    Python Shaper (additive synth, voice_on/voice_off).
...
try:
    import mido
    from mido import ports as mido_ports
    HAS_MIDO = True
except ImportError:
    HAS_MIDO = False
    mido = None
...
class LaunchpadMiniControl:
    """Launchpad Mini → SC + Shaper voice_on/voice_off for 32 harmonics.
    ...
    def panic(self):
        """Used by the Shaper when /beacon/panic arrives."""
        self._clear_all_voices()
        self._all_lights_off()
